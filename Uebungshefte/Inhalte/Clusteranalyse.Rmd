...
```{r}
segment <- read.csv2("segment.csv", stringsAsFactors = TRUE)
```

Zusätzlich wurde hier die Option `stringsAsFactors = TRUE` gesetzt, so dass die Zeichenketten-Variablen (`chr`) als Faktoren eingelesen werden.^[Notwendig ab R-Version 4.0.0.] Dies ist für weitere Funktionen, die in diesem Kapitel angewendet werden, notwendig.
