...
qplot(x = carrier, y = arr_delay, geom = "boxplot", 
      data = flights)
>>>>>>> 39aa425d7ea7e83e7788d4092737a360a6384853
```
...
<<<<<<< HEAD
download.file(
  "https://raw.githubusercontent.com/sebastiansauer/modar/master/datasets/wo_men.csv", 
  "wo_men.csv")
=======
...
Wir weisen wieder der X-Achse und der Y-Achse eine Variable zu; wir sollten aber noch die Extremwerte herausnehmen:
...
Bei sehr großen Datensätzen sind Punkte unpraktisch, da sie sich überdecken ("overplotting"). Eine Abhilfe ist es, die Punkte nur "schwach" zu färben. Dazu stellt man die "Füllstärke" der Punkte über `alpha` ein: `gf_point(alpha = 1/100)`. Um einen passablen Alpha-Wert zu finden, bedarf es häufig etwas Probierens. Zu beachten ist, dass es mitunter recht lange dauert, wenn `ggplot` viele (>100.000) Punkte malen soll.
...
Mit `color = ~sex` sagen wir, dass die Linienfarbe (die Punkte) entsprechend der Ausprägungen von `sex` eingefärbt werden sollen. Die genaue Farbwahl übernimmt `ggplot2` für uns.

### Eine diskrete Variable
Bei diskreten, vor allem nominalen Variablen, geht es in der Regel darum, Häufigkeiten auszuzählen. Wie viele Männer und Frauen sind in dem Datensatz?

```{r}
gf_histogram(~ sex, data = wo_men, stat = "count")
```

Für ein Balkendiagramm bei einer Faktor- oder Textvariable muss hierbei der Parameter `stat` als "count" angegeben werden.
...
wo_men2 %>% 
  ggplot() +
  aes(x = sex, fill = groesse_gruppe, alpha = 0.7) +
...
Für weiterführende Informationen und Hintergründe sei im Folgenden auf die Literatur verwiesen:
