...
Die im Modell angegebenen Schätzwerte sind die Änderung der Trinkgeldprognose, wenn z.B. der Tag ein Samstag (`daySat`) im Vergleich zu einer Referenzkategorie ist. Dies ist in R das erste Element des Vektors -- der Faktorlevel. Welcher dies ist, ist über den Befehl `levels()` zu erfahren:
```{r}
levels(tips$day)
```
