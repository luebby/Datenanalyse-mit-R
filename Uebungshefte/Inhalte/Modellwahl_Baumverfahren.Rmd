---
title: "Baumverfahren und Modellwahl in R"
author: "Karsten Lübke, Matthias Gehrke"
date: ''
output:
...
Die Funktion `partimat()` aus dem Paket `klaR` ermöglicht einen visuellen Vergleich der Auteilungen von `lda()` und `rpart()`, hier nur für die Variablen `CP91JW + IAU91JW`. Die Option `gs=c("1",  "2", "3", "4"))` legt die numerische Codierung der Phasen als Symbol für die Beobachtungen fest. `app. error rate` ist die (Trainings-)Fehlklassifikationsrate mit den zwei unabhängigen Variablen.
