...
Die Koeffzienten werden dabei so geschätzt, dass $\sum \epsilon_i^2$ minimiert wird. Daher spricht man auch vom *Kleinste Quadrate* Kriterium. Eine robuste Regression ist z. B. mit der Funktion `rlm()` aus dem Paket `MASS` möglich.
...
qplot(x = resid.LinMod.1., data = resid_df)
```
...
Außerdem kann man testen, ob alle Koeffizienten der der unabhängigen Variablen gleich Null sind:
$$H_0: \beta_1=\beta_2=\cdots=\beta_k=0$$
Das Ergebnis des zugrundeliegenden F-Tests (vlg. Varianzanalyse) wird in der letzten Zeile angegeben (`F-Statistic`). Hier wird $H_0$ also verworfen.
...
step(LinMod.4)
```
In den letzten Zeilen der Ausgabe steht das beste Modell, dass diese Methode (schrittweise, rückwärts) mit diesem Kriterium (AIC) bei diesen Daten findet (Punktprognose, d. h. ohne Residuum):
...
```
d.h. die geschätzte Funktion ist eine "umgedrehte Parabel" (negatives Vorzeichen bei `I(total_bill^2) `), d. h. die Funktion ist konkav, die Steigung nimmt ab. Allerdings ist der Effekt nicht signifikant.
...
Sie sehen, dass 95\% Prognoseintervall ist recht breit: über den gewählten Rechnungsberich von 0-75\$ im Mittelwert bei 4.11\$. 
