...
Das *simulierte* einseitige 95% Konfidenzintervall geht also von
```{r}
sort(FB1000$mean)[floor(1000*0.05)]
```
bis
```{r}
sort(FB1000$mean)[ceiling(1000*1)]
```
...
Sie können in das Histogramm auch zusätzlich das Konfidenzintervall einzeichnen:
```{r}
ki <- c(sort(FB1000$mean)[floor(1000*0.05)],sort(FB1000$mean)[ceiling(1000*1)])
histogram(~FB1000, v=c(mFB,ki))
```
...
Der (umfangreichen) Ausgabe können Sie neben dem z- bzw. t-Wert (`r round(ptb3$statistic, 2)`) mit unter der Nullhypothese der Gleichheit des Lageparameters $$H_0: \mu_{\text{IB91JW}-\text{IAU91JW}}=0$$ insbesondere den p-Wert (`r round(ptb3$p.value, 4)`) und das Konfidenzintervall (`r round(ptb3$conf.int,2)`) entnehmen. Zum Signifikanznvieau von 5% wird die Nullhypothese also gerade so *nicht* abgelehnt, da der p-Wert über 5\% liegt sowie der Wert der Nullhypothese, $\mu=0$, im Konfidenzintervall.
