...
Der (umfangreichen) Ausgabe kann man neben den z- bzw. t-Wert (`r round(ptb3$statistic, 2)`) unter der Nullhypothese der Gleichheit des Lageparameters $$H_0: \mu_{\text{IB91JW}}=\mu_{\text{IAU91JW}} \Leftrightarrow \mu_{\text{IB91JW}}-\mu_{\text{IAU91JW}}=0$$ insbesondere den p-Wert (`r round(ptb3$p.value, 4)`) und das Konfidenzintervall (`r round(ptb3$conf.int,2)`) entnehmen. Zum Signifikanznvieau von 5% wird die Nullhypothese also gerade so *nicht* abgelehnt, da der p-Wert über 5% liegt.
...
$$H_0: \mu_{\text{Aufschwung}} \geq \mu_{\text{Abschwung}} \quad vs. \quad H_0: \mu_{\text{Aufschwung}} < \mu_{\text{Abschwung}}$$
