...
Die theoretische Wahrscheinlichkeit eines Gewinns liegt bei 18/37=`r round(18/37, 2)`: Achtung, dass Gesetz der großen Zahl gilt für den Durchschnitt und auf lange Sicht, evtl. Ungleichgewichte, z. B. 5 Gewinne in Folge werden im Laufe der Zeit abgeschwächt, und nicht durch anschließende 5 Verluste ausgeglichen.
...
```
`r sum(farbspiele$Verlust==1)`-mal nur verloren, d. h. 8 von 8 Spielen.
>>>>>>> master
...
Das Chancenverhältnis, das **Odds Ratio** liegt bei (79/(151-79))/(79/(143-78))=`r round((79/(151-79))/(79/(143-78)), 2)`, dass ein alleinger Entscheider positiv einstuft -- im Vergleich zum vorläufigen Entscheider.

Zunächst erzeugen wir einen Vektor der Länge 2 mit den Entscheidungstypen, aus dem wir simulieren können:

Im Paper *Hose, C., Lübke, K., Nolte, T., und Obermeier, T. (2012): Ratingprozess und Ratingergebnis: Ein Experiment bei qualitativen Ratingkriterien, Kredit & Rating Praxis (6), 12-14* 

wird folgendes Experiment untersucht: unterscheidet sich die Einschätzung (Rating) eines Unternehmens, je nach dem, ob man alleiniger Entscheider (Typ A), oder derjenige ist, der die  Entscheidungsvorlage vorbereitet (Typ B). Im Rahmen des Experiments wurde die Studierenden zufällig den verschiedenen Typen A und B zugeordnet. Von 151 alleinigen Entscheidern (Typ A) beurteilten 79 das Beispielunternehmen überwiegend positiv (++, +), von 143 Entscheidungsvorlagenerstellern (Typ B) entschieden ebenfalls 79 überwiegend positiv. 

Zeigt das unterschiedliche Verhältnis: Typ A: 79/151=`r round(79/151*100,2)`% zu Typ B: 79/143=`r round(79/143*100,2)`%, dass alleinige Entscheider die Bonität kritischer einstufen, oder könnte das Ergebnis Zufall sein?

Das Chancenverhältnis, das **Odds Ratio** liegt demnach bei (79/(151-79))/(79/(143-78))=`r (79/(151-79))/(79/(143-78))`, dass ein alleinger Entscheider positiv einstuft -- im Vergleich zum vorläufigen Entscheider.
...
```
Hier wären also zufällig `r tally(~simentscheidung)["A"]` der 158 positiven Entscheidungen den Typ A zugeordnet geworden.
...
```
Unter der **Nullhypothese**, dass das Ergebnis zufällig war, wurden in der Simulation in `r round(mean(typsim$B>=79)*100,0)`% der Fälle mindestens 79 dem Typ B zugeordnet. Dieser **p-Wert** spricht also nicht wirklich gegen das Zufallsmodell.
