...
![plot of chunk unnamed-chunk-1](../../Installation/RStudio-Screenshot.png)
...
Zunächst laden wir die Daten als `csv` Datei von der richtigen Adresse (ÙRL`) herunter:

```r
tips <- read.csv2(url("https://raw.githubusercontent.com/luebby/Datenanalyse-mit-R/master/Daten/tips.csv"))
```

```
## Error in open.connection(file, "rt"): Couldn't resolve host name
```
...
![plot of chunk unnamed-chunk-10](tips-Enviroment.png)
...
Mit Hilfe des`$` Operators kann auf einzelne Variablen zugegriffen werden:
