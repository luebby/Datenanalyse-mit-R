...
Wir sehen anhand des Scree-Plots, dass bei den Brand-Rating-Daten die zusätzlich erfasste Streuung nach der dritten Komponente nicht mehr wesentlich zunimmt. 

### Elbow-Kriterium

Nach diesem Kriterium werden alle Hauptkomponenten berücksichtigt, die links von der Knickstelle im Scree-Plot liegen. Gibt es mehrere Knicks, dann werden jene Hauptkomponenten ausgewählt, die links vom rechtesten Knick liegen. Gibt es keinen Knick, dann hilft der Scree-Plot nicht weiter. Bei den
Brand-Rating-Daten tritt der Ellbogen, je nach Betrachtungsweise (ist bei 4 auch noch ein Knick?), entweder bei drei oder vier Komponenten auf. Dies deutet darauf hin, dass die ersten drei oder vier Komponenten die meiste Streuung in den Markendaten erklären.

### Eigenwert-Kriterium (oder Kaiser-Kriterium)

Zusätzlich (oder alternativ) kann das Eigenwert-Kriterium genutzt werden: 
   $$ \textrm{Eigenwert} (=\textrm{erfasste Varianz}) > 1. $$
Dies spricht für drei Hauptkomponenten.
...
Die Adjektiv-Gruppierungen auf den Variablen sind als rote Ladungspfeile sichbar. ZUsätzlich erhalten wir einen Einblick in die Bewertungscluster (als dichte Bereiche von Beobachtungspunkten). Der Biplot ist im Beispiel durch die große Anzahl an Beobachtung recht unübersichtlich. 
